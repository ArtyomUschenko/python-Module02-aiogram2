...
async def load_user_role_content_manager(callback_query: types.Message):
        ...
        if check == "admin":
            ...
            async with state.proxy() as data:
                ...
                if int_user_id < 0:
                    ...
                    await FSM_create_user_role_content_manager.next()
...
@dp.message_handler(state=FSM_create_user_role_content_manager.user_name)
...
#                 await state.finish()
#                 await bot.send_message(message.from_user.id,
#                                        f"Введите корректный id пользователя \n Вы вели следующий ID: {str_user_id}",
#                                        reply_markup=admin_panel_keyboard_back_to_main_menu)
#
#             else:
#                 await state.finish()
#                 await bot.send_message(message.from_user.id, f"Контент менеджер с ID {data['user_id']} успешно создан",
#                                        reply_markup=admin_panel_keyboard_back_to_main_menu)
#
#         except ValueError:
#             await state.finish()
